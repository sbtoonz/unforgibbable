...
return ConnectionState.Client;
        ...
        {
            var resource = typeof(UnforgibbableMod).Assembly.GetManifestResourceNames().Single
                (s => s.EndsWith(bundleName));
            using var stream = typeof(UnforgibbableMod).Assembly.GetManifestResourceStream(resource);
